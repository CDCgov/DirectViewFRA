...
#     percent neutralization values; it does not use LL4 or Gen5 predictions.
#   * Export one canonical calculated field per method:
#       ll4_calculated_nt50
#       uniroot_calculated_nt50
#   * Write one comprehensive curve-level analysis_results.csv. Human-readable
#     pair identifiers come first, followed by NT50 values, reportability and
#     QC flags, then normalization, fit, warning, coefficient, and version data.
#     Multiple flags are separated by a semicolon and a following space.
#   * Use one prespecified primary reporting method for the complete run. Both
#     calculated estimates remain available for review, but the script does not
#     switch reporting methods for selected curves.
...
HARDCODED_SETTINGS <- list(
  ...
  normalization = list(
    ...
    # Cross-plate same-virus VCs are an explicit opt-in.
    ...
    if (branch$layout == "individual") {
      ...
      for (i in seq_along(viruses)) {
        ...
        path <- file.path(
          ...
          paste0(
            sprintf("%03d__", i),
            safe_filename(virus_name, "faceted"),
            ".",
            branch$format
          )
        )
