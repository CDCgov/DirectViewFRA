...
## Plate 09 diagnostic normalization

The example includes a Plate 09 curve for V3X2_H1N2v Example 3 without a plate-matched virus control. Under the default settings, allow_cross_plate_virus_controls = FALSE, so no cross-plate denominator is selected: this curve has no usable virus-control reference, is flagged virus_control_reference_unavailable, and is not reportable. To demonstrate cross-plate diagnostic normalization instead, set allow_cross_plate_virus_controls = TRUE; same-virus controls from other plates are then used for diagnostic normalization and plotting, while allow_cross_plate_vc_for_reporting = FALSE keeps that denominator from producing a reportable result.
